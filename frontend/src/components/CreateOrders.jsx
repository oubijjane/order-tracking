...
import {InputField, Dropdown, SearchableDropdown } from "./Input";
...
import { WINDOW_TYPES, formatOrderPayload2 } from '../utils/formUtils'; // Import helpers
import {
...
function Form() {
    ...
    try {
        const payload = formatOrderPayload2(data);
        ...
        // 3. Send to Service
        await OrderService.createOrders(payload, processedImages);    
        navigate('/');
    } catch (error) {
        ...
        <FormProvider {...methods}>
            <form onSubmit={methods.handleSubmit(onValidSubmit)} className="form-content" noValidate>
                ...
                <Dropdown 
                    ...
                    options={WINDOW_TYPES} multiple={true}
                />  
                <button className={isUpdating ? "disabled-button" : "enabled-button"} type="submit">Crée une commande</button>
            </form>
        </FormProvider>
    );
}
